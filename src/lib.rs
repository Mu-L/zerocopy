...
impl<T, A> Align<T, A> {
    ...
    pub fn try_from_ref<'a>(t: &'a T) -> Option<&'a Align<T, A>> {
        if mem::size_of::<Align<T, A>>() > mem::size_of::<T>()
            || !aligned_to(t, mem::align_of::<Align<T, A>>())
        {
            return None;
        }
    ...
    /// 0`.
    pub fn try_from_mut<'a>(t: &'a mut T) -> Option<&'a mut Align<T, A>> {
        if mem::size_of::<Align<T, A>>() > mem::size_of::<T>()
            || !aligned_to(&*t, mem::align_of::<Align<T, A>>())
        {
            return None;
    ...
    /// condition will happen if `mem::size_of::<T>() > 0` and
    /// `mem::size_of::<T>()` is not a multiple of `mem::align_of::<A>()`.
    pub const unsafe fn from_ref_unchecked<'a>(t: &'a T) -> &'a Align<T, A> {
    ...
    pub unsafe fn from_mut_unchecked<'a>(t: &'a mut T) -> &'a mut Align<T, A> {
        // SAFETY: The caller is responsible for ensuring size and alignment.
        // Since `Align`'s layout is equivalent to `T`'s followed by padding, if
        // there are no padding bytes, then `Align`'s layout is equivalent to
        // `T`'s.
...
impl<T, A> Align<[T], A> {
    ...
    /// condition will happen if `mem::size_of::<T>() > 0` and
    /// `mem::size_of::<T>()` is not a multiple of `mem::align_of::<A>()`.
    pub fn try_from_slice<'a>(t: &'a [T]) -> Option<&'a [Align<T, A>]> {
        if mem::size_of::<Align<T, A>>() > mem::size_of::<T>()
            || !aligned_to(t, mem::align_of::<Align<T, A>>())
        {
            return None;
    ...
    /// condition will happen if `mem::align_of::<A>() > mem::size_of::<T>() >
    /// 0`.
    pub fn try_from_slice_mut<'a>(t: &'a mut [T]) -> Option<&'a mut [Align<T, A>]> {
        if mem::size_of::<Align<T, A>>() > mem::size_of::<T>()
            || !aligned_to(&*t, mem::align_of::<Align<T, A>>())
        {
            return None;
    ...
    /// condition will happen if `mem::size_of::<T>() > 0` and
    /// `mem::size_of::<T>()` is not a multiple of `mem::align_of::<A>()`.
    pub const unsafe fn from_slice_unchecked<'a>(t: &'a [T]) -> &'a [Align<T, A>] {
        ...
        let ptr = t.as_ptr() as *const Align<T, A>;
        let len = t.len();
    ...
    // TODO(https://github.com/rust-lang/rust/issues/57349): Make this `const`
    // once mutable references in const contexts are stable.
    pub unsafe fn from_mut_slice_unchecked<'a>(t: &'a mut [T]) -> &'a mut [Align<T, A>] {
        // SAFETY: The caller is responsible for ensuring size. The caller is
        // also responsible for ensuring that `t` is aligned to `Align<T, A>`.
        // The alignment requirement of `[Align<T, A>]` is the same as that of
        // `Align<T, A>`. Taken together, this means that `t` points to a slice
        ...
        let ptr = t.as_mut_ptr() as *mut Align<T, A>;
        let len = t.len();
...
impl<T: FromBytes, A> Align<T, A> {
    ...
    pub fn from_bytes<'a>(bytes: &'a Align<ByteArray<T>, A>) -> &'a Align<T, A> {
...
impl<T: FromBytes + AsBytes, A> Align<T, A> {
    ...
    /// Since `bytes` is guaranteed to have the same alignment as `Align<T, A>`,
    /// this operation is infallible.
    pub fn from_bytes_mut<'a>(bytes: &'a mut Align<ByteArray<T>, A>) -> &'a mut Align<T, A> {
        // SAFETY: `size_of::<ByteArray<T>>() == size_of::<T>()`. Thus,
        // `Align<ByteArray<T>, A>` and `Align<T, A>` have the same size and
        // alignment, and `ByteArray<T>` and `T` are at the same byte range
        // within `Align`. Since a) `T: FromBytes`, b) `ByteArray<T>` is
...
impl<T> ByteArray<T> {
    ...
    pub unsafe fn from_slice_unchecked(bytes: &[u8]) -> &ByteArray<T> {
        ...
        // `ByteArray<T>`.
        &*(bytes.as_ptr() as *const ByteArray<T>)
    }
    ...
    pub unsafe fn from_mut_slice_unchecked(bytes: &mut [u8]) -> &mut ByteArray<T> {
        ...
        // reference.
        &mut *(bytes.as_mut_ptr() as *mut ByteArray<T>)
    }
...
impl<'a, T> TryFrom<&'a mut [u8]> for &'a mut ByteArray<T> {
    ...
    fn try_from(bytes: &'a mut [u8]) -> Result<&'a mut ByteArray<T>, TryFromSliceError> {
        ...
        // verified.
        unsafe { Ok(&mut *(bytes.as_mut_ptr() as *mut ByteArray<T>)) }
    }
}
...
mod tests {
    ...
    use core::{cmp, convert::Infallible as Never, ops::Deref};
